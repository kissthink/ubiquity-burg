...
# edit by kobe: there is a free partition or not
FREE_HAVE = False
...
class PageGtk(PageBase):
        ...
        if self.current_page == self.page_ask:
            ...
            if free:
                ...
                if biggest_free:
                    ...
                    swap = self.controller.dbfilter.swap_size
                    ins = self.controller.dbfilter.installation_size
                    need_s = str((swap + ins) / 1000000000)
                    display_text = '分区大小至少要为' + need_s + 'GB'
                    text1 = '空闲分区空间不足'
                    text2 = '\n\n请增大空间或选择其他安装类型'
                    context = '%s %s %s' % (text1, display_text, text2)
                    self.controller.dbfilter.frontend.error_dialog(context)
                    return True
            # 升级安装
            if update:
                self.controller.switch_to_install_interface()
    ...
    def set_autopartition_options(self, options, extra_options):
        ...
        opt_widget_manual = getattr(self, 'custom_partitioning')
        opt_desc_manual = getattr(self, 'custom_partitioning' + '_desc')
        opt_widget_manual.show()
        opt_desc_manual.show()
        opt_widget_manual.set_label(options['manual'].title)
        opt_desc_manual.set_markup(fmt % options['manual'].desc)
        opt_desc_manual.set_sensitive(False)
        # 设置高级安装为默认按钮
        opt_widget_manual.set_active(True)
        # 卸载Kylin教学专网系统并重新安装（2个系统时，且一个是教学网）
        if 'replace' in options:
            opt_widget_rp = getattr(self, 'replace_partition')
            opt_desc_rp = getattr(self, 'replace_partition' + '_desc')
            opt_widget_rp.show()
            opt_desc_rp.show()
            opt_widget_rp.set_label(options['replace'].title)
            opt_desc_rp.set_markup(fmt % options['replace'].desc)
            opt_desc_rp.set_sensitive(False)
        else:
            title = '覆盖原有教学网系统'
            desc1 = '注意：这会删除原有教学网系统的所有文件，'
            desc2 = '可能会用完硬盘的所有剩余空间。'
            desc = desc1 + desc2
            opt_widget_rp = getattr(self, 'replace_partition')
            opt_desc_rp = getattr(self, 'replace_partition' + '_desc')
            opt_widget_rp.show()
            opt_desc_rp.show()
            opt_widget_rp.set_label(title)
            opt_desc_rp.set_markup(f % desc)
            opt_widget_rp.set_sensitive(False)
            opt_desc_rp.set_sensitive(False)
        # 利用空闲分区安装 ${DISTRO} ${VER}"
        if 'resize' in options:
            opt_widget_rs = getattr(self, 'resize_use_free')
            opt_desc_rs = getattr(self, 'resize_use_free' + '_desc')
            opt_widget_rs.show()
            opt_desc_rs.show()
            opt_widget_rs.set_label(options['resize'].title)
            opt_desc_rs.set_markup(fmt % options['resize'].desc)
            opt_desc_rs.set_sensitive(False)
            # 不存在空闲分区时，禁用该按钮
            ...
            opt_widget_rs.show()
            opt_desc_rs.show()
            opt_widget_rs.set_label(title)
            opt_desc_rs.set_markup(f % desc)
            opt_widget_rs.set_sensitive(False)
            opt_desc_rs.set_sensitive(False)
    ...
    def partman_dialog(self, devpart, partition, create=True):
        ...
        # -------edit by kobe: size of a new partition-------
        ...
        if max_size_mb is not 0:
            ...
            # ---------edit by kobe----------
            self.size = current_size
            #kobe:no 'max_size_mb is 0',then no self.size
    ...
    def on_partition_use_combo_changed(self, combobox):
        ...
        if show and mount_model:
            ...
            # ---------edit by kobe:显示第一个挂载点格式化勾上---------
            self.partition_mount_combo.set_active(0)
            self.partition_edit_format_checkbutton.set_active(True)

    # -------edit by kobe--------
    def on_partition_mount_combo_changed(self, partition):
        self.partition_size_spinbutton.set_sensitive(True)
        if self.create_flag:
            self.partition_size_spinbutton.set_value(self.max_size)
        if self.edit_flag:
            self.partition_size_spinbutton.set_value(float(self.size))
        self.partition_use_combo.set_sensitive(True)
...
class Page(plugin.Plugin):
    ...
    def calculate_operating_systems(self, layout):
        ...
        ubuntu_systems = [x for x in operating_systems
                          if x.find('UKylin') != -1]
        ubuntukylin_neice = [x for x in operating_systems
                             if x.find('UbuntuKylin for jxzw') != -1]
        kylin_systems = [x for x in operating_systems
                         if x.lower().find('neokylin 3.2.1 v1.2') != -1]
        if len(ubuntukylin_neice) != 0:
            ubuntu_systems.append(ubuntukylin_neice)
        if len(kylin_systems) != 0:
            ubuntu_systems.append(kylin_systems)
    ...
    def calculate_autopartitioning_options(self, operating_systems,
        ...
        if os_count == 0:
            ...
            if os_count == 1 and len(ubuntu_systems) == 1:
